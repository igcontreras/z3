...
struct mbp_array_tg::impl {
    ...
    // m_has_store.is_marked(t) if t has a subterm store(v) where v is a variable to be eliminated
    ...
    bool apply() {
        ...
        for (unsigned i = 0; i < terms.size(); i++) {
            ...
            if (is_implicit_peq(term)) {
                ...
                mark_seen(term);
                progress = true;
                e = mk_wr_peq(to_app(term)->get_arg(0), to_app(term)->get_arg(1)).mk_peq();
                m_tg.add_lit(e);
                m_tg.add_eq(term, e);
                continue;
            }
            ...
            if (is_app(nt) && is_partial_eq(to_app(nt))) {
                ...
                if (!m_array_util.is_store(p.lhs()) &&  has_var(p.lhs())) {
                    //TODO: don't apply this rule if vars in p.lhs() also appear in p.rhs()
                    mark_seen(p.lhs());
                    mark_seen(nt);
                    mark_seen(term);
                    progress = true;
...
family_id mbp_array_tg::get_family_id() const { return m_impl->m_array_util.get_family_id(); }
mbp_array_tg::mbp_array_tg(ast_manager& man, mbp::term_graph& tg, model& mdl, obj_hashtable<app> &vars_set, expr_sparse_mark &seen) {
